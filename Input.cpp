...
void Input::Initialize()
{
	...
	// DirectInputの初期化
	if (FAILED(DirectInput8Create(hInstance, DIRECTINPUT_VERSION, IID_IDirectInput8, reinterpret_cast<void**>(directInput.GetAddressOf()), nullptr))) {
		throw std::runtime_error("Failed to create DirectInput object");
	}

	// マウスデバイスの作成
	if (FAILED(directInput->CreateDevice(GUID_SysMouse, devMouse_.GetAddressOf(), nullptr))) {
