...
namespace {
	...
	// 遮蔽率は0〜1の1チャンネル。R8だと半影の階調に段差が見えるのでFP16
	constexpr DXGI_FORMAT kShadowFormat = DXGI_FORMAT_R16_FLOAT;
...
void RayTracedShadowPass::Initialize(DXCom* pDxcom, LightManager* pLightManager) {
	dxcommon_ = pDxcom;
	lightManager_ = pLightManager;

	ID3D12Device* device = dxcommon_->GetDevice();

	raw_.Create(device, kShadowFormat, false);
	denoiser_.Initialize(pDxcom);

	for (uint32_t i = 0; i < kFrameCount_; i++) {
		paramResource_[i] = Helper::CreateBufferResource(device, sizeof(ShadowPassParam));
...
void RayTracedShadowPass::Finalize() {
	gpuTimer_.Finalize();
	denoiser_.Finalize();
...
void RayTracedShadowPass::Render(const GBufferPass& gbuffer) {
	if (dxcommon_ == nullptr || lightManager_ == nullptr || !gbuffer.IsRendered() ||
		lightManager_->GetData().shadowMode != kShadowModeSoft) {
...
void RayTracedShadowPass::DispatchTrace(uint32_t frameIndex, const GBufferPass& gbuffer, uint32_t tlasSrvIndex) {
	...
	pPipeManager->SetCSPipeline(Pipe::RayTracedShadowCS);
	...
	pPipeManager->SetComputeRootCBV(cList, RootName::kShadowPassParam, paramResource_[frameIndex]->GetGPUVirtualAddress());
	...
	pPipeManager->SetComputeRootDescriptorTable(cList, RootName::kShadowOutput, srvManager->GetGPUDescriptorHandle(raw_.uavIndex));
...
void RayTracedShadowPass::DebugGUI() {
#ifdef _DEBUGMODE
	if (!ImGui::TreeNode("RayTraced Shadow (screen space)")) {
		return;
	}
	...
	ImGui::SliderFloat("preview scale##shadow", &scale, 0.1f, 1.0f);
...
void RayTracedShadowPass::UpdateParam(uint32_t frameIndex) {
	frameCounter_++;

	ShadowPassParam param{};
