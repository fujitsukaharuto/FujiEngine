...
void AnimationPipeline::CreatePSO(ID3D12Device* device) {
	...
	vs = DXCom::GetInstance()->GetDXCompil()->CompileShader(kDirectoryPath_ + L"SkinningObject3d.VS.hlsl", L"vs_6_0");
	assert(vs != nullptr);
	ps = DXCom::GetInstance()->GetDXCompil()->CompileShader(kDirectoryPath_ + L"SkinningObject3d.PS.hlsl", L"ps_6_0");
	assert(ps != nullptr);
