...
void UpdateParticleCSPipe::CreateRootSignature(ID3D12Device* device) {
	...
	// UpdateParticleCSに必要なルートパラメータを設定していく
	...
	rootParameters[7].DescriptorTable.pDescriptorRanges = &descriptorRanges[6];
	rootParameters[7].DescriptorTable.NumDescriptorRanges = 1;

	rootParameters[8].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
	rootParameters[8].ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL;
	rootParameters[8].DescriptorTable.pDescriptorRanges = &descriptorRanges[7];
	rootParameters[8].DescriptorTable.NumDescriptorRanges = 1;

	rootParameters[9].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
	rootParameters[9].ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL;
	rootParameters[9].DescriptorTable.pDescriptorRanges = &descriptorRanges[8];
...
void UpdateParticleCSPipe::CreatePSO(ID3D12Device* device) {
	...
	vs = dxcommon_->GetDXCompile()->CompileShader(kDirectoryPath_ + L"CS/Engine/UpdateParticle.CS.hlsl", L"cs_6_0");
	assert(vs != nullptr);
