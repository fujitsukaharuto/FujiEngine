...
void EmitterSurfaceParticleCSPipe::CreateRootSignature(ID3D12Device* device) {
	...
	// EmitSurfaceCSに必要なルートパラメータを設定していく
	...
	descriptorRanges[9].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 1);
	descriptorRanges[10].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 2);
	descriptorRanges[11].Init(D3D12_DESCRIPTOR_RANGE_TYPE_UAV, 1, 8);
	...
	rootParameters[12].DescriptorTable.pDescriptorRanges = &descriptorRanges[10];
	...
	rootParameters[13].DescriptorTable.pDescriptorRanges = &descriptorRanges[11];
...
void EmitterSurfaceParticleCSPipe::CreatePSO(ID3D12Device* device) {
	...
	vs = dxcommon_->GetDXCompile()->CompileShader(kDirectoryPath_ + L"CS/Engine/EmitSurfaceParticle.CS.hlsl", L"cs_6_0");
	assert(vs != nullptr);
