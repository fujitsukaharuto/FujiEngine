...
void Line3dPipe::CreateRootSignature(ID3D12Device* device) {
	...
	auto vsData = dxcommon_->GetDXCompile()->CompileShaderWithReflection(kDirectoryPath_ + L"Line3d.VS.hlsl", L"vs_6_0");
	vs = vsData.blob;
	vsReflection_ = vsData.reflection;
	assert(vs != nullptr);

	auto psData = dxcommon_->GetDXCompile()->CompileShaderWithReflection(kDirectoryPath_ + L"Line3d.PS.hlsl", L"ps_6_0");
	ps = psData.blob;
