...
void PipelineNode::CreateRootSignature(ID3D12Device* device) {
	...
	rootParameters[10].DescriptorTable.pDescriptorRanges = &descriptorRange[2];
...
void PipelineNode::CreatePSO(ID3D12Device* device) {
	...
	assert(vs != nullptr);
	ps = dxcommon_->GetDXCompile()->CompileShader(kDirectoryPath_ + L"Object3dNode.PS.hlsl", L"ps_6_0");
	assert(ps != nullptr);
