#include "LockOn.h"
#include "Game/GameObj/Enemy/Enemy.h"
#include "Input.h"
...
void LockOn::ChangeSearch(const std::list<std::unique_ptr<Enemy>>& enemies, bool isRight) {
	...
	auto it = std::find_if(targets.begin(), targets.end(),
		[this](const std::pair<float, const Enemy*>& pair) {
