...
bool Beam::BeamRotate() {// これは今使ってない後から無くすつもり。別関数作るときの例
	...
	if (lifeTime_ > 360.0f) {
		...
		// ビームの回転
		beam1_->GetTransform().scale.x = std::lerp(0.0f, beam1BaseScale_, t);
		beam1_->GetTransform().scale.z = std::lerp(0.0f, beam1BaseScale_, t);
		beam2_->GetTransform().scale.x = std::lerp(0.0f, beam2BaseScale_, t);
		beam2_->GetTransform().scale.z = std::lerp(0.0f, beam2BaseScale_, t);
		beam3_->GetTransform().scale.x = std::lerp(0.0f, beam3BaseScale_, t);
		beam3_->GetTransform().scale.z = std::lerp(0.0f, beam3BaseScale_, t);
		beamCore1_->GetTransform().scale.x = std::lerp(0.0f, beam2BaseScale_ * 2.0f, t);
		beamCore1_->GetTransform().scale.z = std::lerp(0.0f, beam2BaseScale_ * 2.0f, t);
		beamCore2_->GetTransform().scale.x = std::lerp(0.0f, beam2BaseScale_ * 2.05f, t);
		beamCore2_->GetTransform().scale.z = std::lerp(0.0f, beam2BaseScale_ * 2.05f, t);
		beamCore3_->GetTransform().scale.x = std::lerp(0.0f, beam2BaseScale_ * 2.05f, t);
		beamCore3_->GetTransform().scale.z = std::lerp(0.0f, beam2BaseScale_ * 2.05f, t);
		beamParticle_->Emit();
		beamLight_->Emit();
		...
		beam1_->GetTransform().scale.x = std::lerp(beam1BaseScale_, 0.0f, t);
		beam1_->GetTransform().scale.z = std::lerp(beam1BaseScale_, 0.0f, t);
		beam2_->GetTransform().scale.x = std::lerp(beam2BaseScale_, 0.0f, t);
		beam2_->GetTransform().scale.z = std::lerp(beam2BaseScale_, 0.0f, t);
		beam3_->GetTransform().scale.x = std::lerp(beam3BaseScale_, 0.0f, t);
		beam3_->GetTransform().scale.z = std::lerp(beam3BaseScale_, 0.0f, t);
		beamCore1_->GetTransform().scale.x = std::lerp(beam2BaseScale_ * 1.5f, 0.0f, t);
		beamCore1_->GetTransform().scale.z = std::lerp(beam2BaseScale_ * 1.5f, 0.0f, t);
		beamCore2_->GetTransform().scale.x = std::lerp(beam2BaseScale_ * 1.75f, 0.0f, t);
		beamCore2_->GetTransform().scale.z = std::lerp(beam2BaseScale_ * 1.75f, 0.0f, t);
		beamCore3_->GetTransform().scale.x = std::lerp(beam2BaseScale_ * 1.75f, 0.0f, t);
		beamCore3_->GetTransform().scale.z = std::lerp(beam2BaseScale_ * 1.75f, 0.0f, t);
...
void Beam::SetBossParent(Boss* boss) {
	model_->SetParent(&boss->GetAnimeModel()->GetTransform());
	model_->SetNoneScaleParent(true);
	particleParent_->SetParent(&boss->GetAnimeModel()->GetTransform());
	particleParent_->SetNoneScaleParent(true);
	for (auto& beam : beams_) {
		beam.model->SetParent(&boss->GetAnimeModel()->GetTransform());
		beam.model->SetNoneScaleParent(true);
		beam.particleParent->SetParent(&boss->GetAnimeModel()->GetTransform());
