...
float simplex3d(float3 v)
{
    ...
    float3 g0 = float3(gx.x, gy.x, gx.x);
    float3 g1 = float3(gx.y, gy.y, gx.y);
    float3 g2 = float3(gx.z, gy.z, gx.z);
    float3 g3 = float3(gx.w, gy.w, gx.w);

    return dot(g0, x) * gw.x +
           dot(g1, x1) * gw.y +
           dot(g2, x2) * gw.z +
           dot(g3, x3) * gw.w;
}
...
float3 CurlNoise(float3 p)
{
    ...
    float3 p_x0 = Noise3(p - dx);
    float3 p_x1 = Noise3(p + dx);
    float3 p_y0 = Noise3(p - dy);
    float3 p_y1 = Noise3(p + dy);
    float3 p_z0 = Noise3(p - dz);
    float3 p_z1 = Noise3(p + dz);
