...
void RightPipe::CreateRootSignature(ID3D12Device* device) {
...
void RightPipe::CreatePSO(ID3D12Device* device) {
	...
	vs = DXCom::GetInstance()->GetDXCompil()->CompileShader(kDirectoryPath_ + L"Right.VS.hlsl", L"vs_6_0");
	assert(vs != nullptr);
	ps = DXCom::GetInstance()->GetDXCompil()->CompileShader(kDirectoryPath_ + L"Right.PS.hlsl", L"ps_6_0");
	assert(ps != nullptr);
